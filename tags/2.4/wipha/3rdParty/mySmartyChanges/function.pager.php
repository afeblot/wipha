...
if (!empty($tmp)) {
    $url = $_SERVER['SCRIPT_NAME'].'?'.substr($tmp, 5);
    $link = '&amp;';
} else {
...
// image url
print '<img src="'.$img_first.'" border="0" alt="first" />';
}
} else {
print $txt_first;
}
...
// image url
$cache['prev'] .= '<img src="'.$img_prev.'" border="0" alt="prev" />';
}
} else {
$cache['prev'] .= $txt_prev;
}
...
// image url
$cache['next'] .= '<img src="'.$img_next.'" border="0" alt="next" />';
}
} else {
$cache['next'] .= $txt_next;
}
