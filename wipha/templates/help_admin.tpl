...
<ul class="sum">
    ...
    <li><a href="#newacc">CREATING NEW ACCESSES</a></li>
...
<p> Give these login/password and click "Login"; then click the "Admin" item at the right on the
WiPhA menu bar to reach the main admin page.</p>
...
<p>You must now add at least one iPhoto library to share. Give it a name, and enter the
<strong>absolute</strong> path to the library folder (as an example, here's the default
location, with <tt>&lt;user&gt;</tt> being as always your short user name:
<tt>/Users/&lt;user&gt;/Pictures/iPhoto&nbsp;Library</tt>). Each added library may be edited
later to change it's name or path; just click it's name in the list to get the library
modification (rather simple) page.</p>
...
<p><img src="img/delete.gif" alt="Delete"/>&nbsp;and the red cross deletes the corresponding
library (without asking any confirmation).</p>


<a name="newacc"><h2>CREATING NEW ACCESSES</h2></a>

<div class="image">
<img src="img/admuserlist.jpg" alt="Admin Users List"/>
</div>

<p>Then, you may create as many users as you want. For each of them, you will define which iPhoto
albums he/she will be allowed to access.<br/>
To add a user, just type a login and a password in the third frame named <em>Users</em>. It will
be added in the users list shown below. Then, you will have to provide your user with this login
and password.</p>

<div class="note">There is also a "default" user named Guest which does not require any login. This is the
public access so you may decide not to allow any album for Guest, or only some not too private
albums.</div>

<div class="warn">If you Safari user applied a trick like <a
href="http://www.macosxhints.com/article.php?story=20041012233334948" target="_blank">this one</a>
to prevent the use of the <tt>autocomplete="off"</tt> option in login/password fields, you
will let Safari overwrite WiPhA default values; as a result, you probably won't understand the
behaviour of these admin pages and you'll damage your own previous settings.</div>

<a name="confacc"><h2>CONFIGURING THESE ACCESSES</h2></a>
...
<p>And at last, here's the interesting part: Select which albums <strong>of the currently loaded
library</strong> John  will be allowed to see. When you've done, don't forget to click the
"Change" button. If you share more than one library, you have to load each of them, and define
the rights for each. This is where the <img src="img/select.gif" alt="Select"/> arrow gets
useful. So, the fatest recommended way is to choose the first library, set the rights for all
users, load the second library, set the rights, and so on...</p>

<div class="note">The first album item "<i>Allow all albums in this library</i>" let you give
some users the full access to this library, including all future albums to be created.</div>
